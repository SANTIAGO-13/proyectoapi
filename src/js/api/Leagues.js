...
const LEAGUES = {
	esp: {
		id: 6,
		index: 1,
		video: "https://www.youtube.com/embed/EZcv3RTUu9U?si=AaX7br7WZai6IIKa",
	},
	por: {
		id: 92,
		index: 0,
		video: "https://www.youtube.com/embed/sAxOxd7oLLI?si=fMBArq5YEm0kSPaH",
	},
	ita: {
		id: 5,
		index: 1,
		video: "https://www.youtube.com/embed/OjAvJ8bq3x4?si=WUfIMWH6_C_0wgsb",
	},
};
const EVENTS = {
	esp: 302,
...
function createHtml(urlImg, textPais, textLiga, urlVideo = "") {
	const videoFrame = urlVideo
		? `<iframe width="560" height="315" src=${urlVideo} frameborder="0" allow="autoplay; encrypted-media; picture-in-picture" allowfullscreen></iframe>`
		: "";
...
async function showLeagues(live = false) {
	...
	for (const league in LEAGUES) {
		...
		const leagueData = await fetchAPI(
			`${API_EVENTS_URL}?action=get_leagues&country_id=${id}&APIkey=${KEY}`
		);
...
async function showMatches(league, leagueId) {
	const matches = await fetchAPI(
		`${API_EVENTS_URL}?action=get_events&league_id=${leagueId}&APIkey=${KEY}&from=2024-09-15&to=2024-09-15`
	);

	if (matches && matches.length > 0) {
		matches.forEach((match) => {
			const matchHtml = createMatchHtml(
				match.match_hometeam_name,
				match.match_awayteam_name,
				match.match_hometeam_score,
				match.match_awayteam_score,
				"../pages/analisticas.html"
			);
